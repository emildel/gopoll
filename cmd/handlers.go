...
func (app *application) createPollPOSTWithSession(w http.ResponseWriter, r *http.Request) {
	...
	err := r.ParseForm()
	if err != nil {
		app.logger.Error("error parsing form when trying to update poll", "pollId", pollId, "error", err)
	...
	if err != nil {
		...
		case data.ErrDuplicateKey:
			...
			return
		default:
			app.logger.Error("error inserting poll to database", "pollId", pollId, "error", err)
			app.serverErrorResponse(w, r, err)
			return
		}
	}
	...
	app.logger.Info("Poll created", "pollId", pollId)
...
func (app *application) answerPoll(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		app.logger.Error("error updating poll", "pollId", pollId, "error", err)
		app.serverErrorResponse(w, r, err)
		return
	}
